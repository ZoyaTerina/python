...
while True:
    number = input("Сколько дней рождений мы генерируем? (до 70) ")
    if not number.isdigit() or int(number) < 2 or int(number) > 70:
        print("Это по-твоему смешно? Введи от 2 до 70, пж 😒")
        print("-" * 5)
    else:
        number = int(number)
        break  # вызод из while True

birthdays = []
startOfYear = datetime.date(2010, 1, 1)
...
print("=" * 10)
for i in set(birthdays): # список -> множество. исключение повторения
